...
이때 디폴트 값은 개발자 의해 명시되지만 타입은 여전히 옵셔널 처리 되어 있어, 타입이 변경되거나 추가 될때 디폴트 값을 제대로 반영하지 못 할 수도 있습니다.  
이런 문제를 해결하기 위해 디폴트 값의 타입은 파라미터 타입의 모든 속성을 `Required` 처리하여  만들어 줄 수 있습니다.  
